...
TNode* getTallerSubTree(TNode* root)
{
  ...
  else
  {
    ...
    {
      return root->pLeft->height >= root->pRight->height ? root->pLeft : root->pRight;
    }
  }
}

 bool isSameSignBalance(TNode* x, TNode* z){
    if(  ((getBalance(x) <= 0) && getBalance(z)<= 0)  ||  (((getBalance(x) >= 0) && getBalance(z)>= 0))  )
    {
      return true;
    }
    else
...
void printHuffmanEncoding( TNode* root, char c ){
    ...
    if (root->pLeft != NULL && root->pRight != NULL)
    {
      ...
      else if ( strchr(root->pRight->str,c) != NULL )
      {
        printf("1");
...
void insertSegment( TNode* root, double segmentStart, double segmentEnd ){
    ...
    if (root == NULL)
    {
      return;
       ...
       else if ((root -> high < segmentStart && root -> high < segmentEnd ) || ( root -> low > segmentEnd && root -> low > segmentStart)){
         return;
       }
