 ...
 * It also has a zoneCode, which is the zone the animal is in.
...
public class Animal {
    ...
    private char zoneCode; // char for the zone code of the animal
    public char getZoneCode() {return zoneCode;}
    public void setZoneCode(char zoneCode) {this.zoneCode = zoneCode;}


    public Animal(String type, String name, boolean Carnivorous, char zoneCode) { // constructor for the animal
    ...
    public String toString() { // toString function for the animal, this will be used for representing the class in a string
        return getName() + " - " + getType() + " (" + (getCarnivorous() ? "Carnivore" : "Vegetarian") + ")"; // _(type)_ - _(name)_ (_(carnivorous)_) // uses a ternary operator to add Carnivore or Vegetarian to the string
    }
}